    ...
    <div class="mb-8">
      <div class="flex items-center justify-between mb-4">
        <h2 class="text-2xl font-bold text-gray-900">
          {{ props.customer ? 'Editar Cliente' : 'Novo Cliente' }}
        </h2>
      ...
      <div v-show="currentStep === 1" class="animate-fade-in">
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <div class="relative">
                ...
                <select 
                  ...
                  :class="{ 'border-red-300': errors.documentType }"
                >
            ...
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-2">
                {{ form.documentType === 'CPF' ? 'CPF' : 'CNPJ' }} <span class="text-red-500">*</span>
              </label>
              <div class="relative">
                <svg class="w-5 h-5 text-gray-400 absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none z-10" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <p v-if="errors.document" class="mt-1 text-sm text-red-600">{{ errors.document }}</p>
            </div>
          </div>
          
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">
              Nome {{ form.documentType === 'CNPJ' ? '/ Razão Social' : '' }} <span class="text-red-500">*</span>
            </label>
            <div class="relative">
              <svg class="w-5 h-5 text-gray-400 absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none z-10" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path>
          ...
          <div class="flex items-center space-x-3 p-4 bg-blue-50 rounded-lg">
            <div class="flex items-center">
              ...
              >
              <label for="cliente-ativo" class="ml-2 text-sm font-medium text-gray-700">
                Cliente Ativo
              </label>
            </div>
            <span class="text-xs text-gray-500">O cliente poderá realizar compras</span>
          </div>
        </div>
      </div>
      ...
      <div v-show="currentStep === 3" class="animate-fade-in">
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          <div class="mb-6">
            ...
            ></textarea>
            <p class="mt-1 text-sm text-gray-500">Máximo de 500 caracteres</p>
          </div>
          ...
          <div class="bg-gray-50 rounded-lg p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              ...
              <div>
                <p class="font-medium text-gray-700">{{ form.documentType }}:</p>
                <p class="text-gray-900">{{ form.document || 'Não informado' }}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p class="text-gray-900">{{ form.email || 'Não informado' }}</p>
              </div>
              <div>
            ...
            <div class="mt-4 flex items-center space-x-2">
              ...
              <span class="text-sm font-medium" :class="form.isActive ? 'text-green-700' : 'text-red-700'">
                {{ form.isActive ? 'Cliente Ativo' : 'Cliente Inativo' }}
              </span>
            </div>
          </div>
        </div>
...
const validateCurrentStep = () => {
  ...
  if (currentStep.value === 1) {
    ...
    if (!form.documentType) {
      errors.documentType = 'Tipo de pessoa é obrigatório'
      isValid = false
    }
    
    if (!form.document) {
      errors.document = form.documentType === 'CPF' ? 'CPF é obrigatório' : 'CNPJ é obrigatório'
      isValid = false
    } else if (form.documentType === 'CPF' && form.document.replace(/\D/g, '').length !== 11) {
      errors.document = 'CPF inválido'
      isValid = false
    } else if (form.documentType === 'CNPJ' && form.document.replace(/\D/g, '').length !== 14) {
...
const isStepCompleted = (step) => {
  if (step === 1) {
    return form.name && form.document && form.phone && form.documentType
  }
  if (step === 2) {
    return form.addressZipCode && form.addressStreet && form.addressNumber && 
...
const saveCustomer = async () => {
  ...
  try {
    ...
    if (response.success) {
      ...
      toast.error(response.error || 'Erro ao salvar cliente')
    }
  } catch (error) {
